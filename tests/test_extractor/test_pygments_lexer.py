...
def _kinds(src: str) -> list[type]:
    """Return the Pygments token TYPES (not the literal text)."""
    lex = FlipJumpLexer()
    return [tt for tt, _ in lex.get_tokens(src) if not _is_whitespace(tt)]
...
def test_dotted_identifier_classified():
    # `stl.startup` should appear as Name (regular identifier)
    out = _kinds_with_text("stl.startup")
    assert any(tt is Name and txt == "stl.startup" for tt, txt in out)


def test_leading_dot_identifier_classified():
    # `.tables.x` should appear as Name.Other (namespace-relative ref)
